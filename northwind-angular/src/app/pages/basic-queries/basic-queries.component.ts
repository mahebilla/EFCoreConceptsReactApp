/**
 * BASIC QUERIES PAGE — Angular Concept: Page Component Pattern
 *
 * React equivalent: BasicQueries.tsx
 *
 * In Angular, each page is a standalone component that:
 * 1. Defines a demos array (same data as React)
 * 2. Passes it to <app-demo-section> via @Input binding
 *
 * The pattern is identical to React: data-driven pages with a shared renderer.
 */
...
@Component({
  ...
  /**
   * Template — Angular Concept: Component Template
   *
   * React equivalent: return <DemoSection title="..." subtitle="..." demos={demos} />
   *
   * In Angular:
   * - [title]="'...'" — Property binding with string value (note the nested quotes)
   * - [demos]="demos" — Property binding with variable reference
   *
   * The [squareBrackets] tell Angular to evaluate the expression.
   * Without brackets, Angular treats it as a plain string attribute.
   */
  template: `
    <app-demo-section
